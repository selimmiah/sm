...
class ProductController extends Controller
{
    ...
    public function uploadUpdate(Request $request,$id)
    {
        ...
        $img = Image::make(public_path().'/images/products/'.$data->photo)->resize(285, 285);
        $thumbnail = time().str_random(8).'.jpg';
        $img->save(public_path().'/images/thumbnails/'.$thumbnail);
    ...
    public function store(Request $request)
    {
        ...
        // Set Thumbnail
        $img = Image::make(public_path().'/images/products/'.$prod->photo)->resize(285, 285);
        $thumbnail = time().str_random(8).'.jpg';
        $img->save(public_path().'/images/thumbnails/'.$thumbnail);
        ...
        if ($files = $request->file('gallery')){
            foreach ($files as  $key => $file){
                if(in_array($key, $request->galval))
                {
                    $gallery = new Gallery;
                    $name = time().$file->getClientOriginalName();
                    $file->move('images/galleries',$name);
                    $gallery['photo'] = $name;
