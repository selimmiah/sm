...
class ProductController extends Controller
{
    ...
    public function uploadUpdate(Request $request,$id)
    {
        ...
        file_put_contents($path, $image);
                if($data->photo != null)
                {
                    if (file_exists(public_path().'/images/products/'.$data->photo)) {
                        unlink(public_path().'/images/products/'.$data->photo);
                    }
                }
         ...
         $data->update($input);
                ...
                {
                    if (file_exists(public_path().'/images/thumbnails/'.$data->thumbnail)) {
                        unlink(public_path().'/images/thumbnails/'.$data->thumbnail);
                    }
                }
        ...
        $resizedImage = Image::make(public_path().'/images/products/'.$data->photo)->resize(300, 300, function ($c) {
            $c->aspectRatio();
            $c->upsize();
        });
        ...
        $background->save(public_path().'/images/thumbnails/'.$thumbnail);
...
//                $input['childcategory_id'] = $request->childcategory_id;
        ...
        if (!empty($request->meta_tag))
         {
           ...
           if (empty($attrArr)) {
            ...
            // Set SLug
                ...
                // Set Photo
                $resizedImage = Image::make(public_path().'/images/products/'.$prod->photo)->resize(800, null, function ($c) {
                    $c->aspectRatio();
                });
                $photo = time().str_random(8).'.jpg';
                $resizedImage->save(public_path().'/images/products/'.$photo);


                // Set Thumbnail
                $background = Image::canvas(300, 300);
                $resizedImage = Image::make(public_path().'/images/products/'.$prod->photo)->resize(300, 300, function ($c) {
                    $c->aspectRatio();
                    $c->upsize();
                });
                // insert resized image centered into background
            ...
            // Add To Gallery If any
                ...
                if ($files = $request->file('gallery')){
                    ...
                    $thumbnail = time().str_random(8).'.jpg';
                    $img->save(public_path().'/images/galleries/'.$name);
                    $gallery['photo'] = $name;
    ...
    public function update(Request $request, $id)
    {
        ...
        $input['photo'] = $name;
            ...
            {
                if($data->file!=null){
                        if (file_exists(public_path().'/files/'.$data->file)) {
    ...
    public function destroy($id)
    {
        ...
        if($data->galleries->count() > 0)
        {
            foreach ($data->galleries as $gal) {
                    if (file_exists(public_path().'/images/galleries/'.$gal->photo)) {
                        unlink(public_path().'/images/galleries/'.$gal->photo);
                    }
                $gal->delete();
            }
        ...
        if (file_exists(public_path().'/images/thumbnails/'.$data->thumbnail) && $data->thumbnail != "") {
            unlink(public_path().'/images/thumbnails/'.$data->thumbnail);
        }
        if($data->file != null){
            if (file_exists(public_path().'/files/'.$data->file)) {
